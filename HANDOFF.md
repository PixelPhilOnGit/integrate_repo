...
npm test             # 967 个纯逻辑单测（秒级）
npm run test:e2e     # 180 个端到端测试（真实 Chromium，这台机器上跑 6 分半）
...
- **重放字节会把「粘性模式」丢掉 —— 折叠那条路的全部风险就在这儿。**
  ...
  「裸字节重建不出终端状态」。补法是在重放**之后**把当前模式再设一遍
  （`hub.ts` 的 `stickyModes`：DECCKM、bracketed paste、插入模式、原点模式……）。
  丢了这些**不报错**，只是方向键发错序列、粘贴不带包装 —— 用户只会说
  「这个终端怪怪的」。另外两条护栏：**备用屏幕里不重放**（全屏程序那不是
...
1. **凭据存储**（下面那段）—— 别的都是「更好用」，这个是**上生产的硬门槛**。
2. **连接管理**：现在的连接列表是平铺的，连接一多就找不到。要做**分组目录 + 搜索**，
   三个连接模块（Redis / 数据库 / SSH）和智能体会话的工作目录都要。用户已经拍板了。
...
现在**三个模块**（Redis / 数据库 / SSH）的连接密码都是**明文**存的 ——
桌面端落在应用数据目录下 SQLite 的 `kv` 表里（这一轮刚从各自的 json 搬过去），
浏览器版在 localStorage。SSH 还多一个**私钥口令**，是同一张表里的另一个键。
