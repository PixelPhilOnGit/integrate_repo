...
npm test             # 1016 个纯逻辑单测（秒级）
npm run test:e2e     # 189 个端到端测试（真实 Chromium，这台机器上跑 7 分半）
...
3. **智能体会话支持远程会话**（用户 2026-09-20 提的，明确说「优先级低一点，但也要做」）。
   **这条是独立的一整轮，不是顺手能加的东西**，所以 2026-09-20 那轮没有动它。
   动手前先和用户确认一件事：**「远程」指的是连你自己的另一台机器，还是容器 /
   云开发机？** 两者在状态检测上可能是同一套解法，但认证和「那个 shell 是什么」
   差得远 —— 猜错方向就是白干一整轮。

   已经能确定的部分（省下一轮的调研）：
   - **连接那一层可以照抄 SSH 模块**：`devtoolkit-ssh` 已经有 russh 的连接、认证、
     主机密钥（TOFU）、PTY。会话侧 `agent_open` 现在走的是本地 pty（`devtoolkit-agents`
     的 `pty.rs`），远端那条路要的是**同一套 Channel 流式形状**（字节 base64、write 串行）。
   - ⚠️ **状态检测是真正的难点，不是连接**：现在那套靠「往 `~/.claude/settings.json`
     装钩子 + 事件文件」。远端写不了我们的包装脚本（也不该悄悄往别人的机器上写），
     所以远端模式下只剩两条路：**OSC 序列**（`core/osc.ts` 那个扫描器，零配置 ✓）
     和**用户的键盘**。也就是说 **Codex 的「需要你」在远端的 v1 大概率也拿不到** ——
     和本地那个已知缺口一样，别当成 bug。
   - 前端那边 `AgentSession` 要多一个「这是个远端会话」的形状，分屏/状态那些**不用动**。
