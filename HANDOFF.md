...
npm test             # 987 个纯逻辑单测（秒级）
npm run test:e2e     # 184 个端到端测试（真实 Chromium，这台机器上跑 6 分半）
...
2. **连接管理** —— **三个连接模块那半已经做完**（分组目录 + 搜索，见上面「连接分组」
   那一节）。**智能体会话那半还差**：一个工作目录下开了很多会话时只能一个个看，
   要加**会话的搜索/过滤**和**工作目录的分组或置顶**。用户早就拍板过两边都要。
