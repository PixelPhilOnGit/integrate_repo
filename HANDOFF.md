...
⚠️ **前六个可用；助手只有「配置」那部分能用** —— 内核完整且有 145 条测试，
但 HTTP 客户端和对话界面还没接上，主区域明说了这一点。见下面「助手」两节。
...
**现在能用的只有配置。** 图标栏最后一个（Ctrl+7），右侧「模型」面板：
...
npm test             # 1018 个纯逻辑单测（秒级）
npm run test:e2e     # 191 个端到端测试（真实 Chromium，这台机器上跑 6 分半）
...
- **助手内核**（`src-tauri/assistant/`）—— 145 条测试，循环/上下文/审批/provider
  全都跑通了，**而且不需要网络也不需要 key**（假实现 + 自己起的假 HTTP 服务器）。
- **助手模块的配置界面** —— 能填模型、能存 key。**对话还没接。**
- **修掉终端 `clear` 之后残留的命令块色条**，顺带修掉 `ESC[3J` 让块行号失序那条
  （那条的症状是 clear 之后新命令的色条压根出不来）。

**助手剩下的（按顺序）**：

1. **真 tool runner**（`fs.rs` / `exec.rs`）—— 走 `Workspace::resolve()` 那道
   **唯一**的路径闸门。⚠️ 跑命令那条**路径闸门挡不住**（`cd ..` 就出去了），
   所以永远要人工确认；而且它有自己的坑：**PATH 快照**（桌面应用拿到的是登录时的
   PATH，用户后装的东西不在里面，症状是「我自己终端里好好的，agent 跑同样的命令
   就找不到」）。
2. **transcript 落盘**（SQLite，照 `tasks/` 的 `user_version` 迁移模式）。
   每轮把 usage 存进去 —— 那是「哪个上下文策略划算」唯一的数据来源。
   ⚠️ `[profile.release]` 里是 `panic = "abort"`：**每轮结束就 commit**，
   不要攒到最后一次性写。
3. **命令层 + 对话界面**（`assistant_commands.rs` 把 mpsc 适配成 Channel，
   ⚠️ Channel 必须搬进转发任务，否则前端立刻收到 `{end:true}`、界面一片空白
   且不报错）。
4. 之后再谈二期（摘要策略 + 工具面扩到 redis/sql/ssh/tasks）和三期（LanceDB + 检索）。
...
|---|---|
| 前端 `src/` | ~30k |
| Rust `src-tauri/`（含各内核 crate） | ~28k |
| ├ app crate（`src-tauri/src/`） | ~2k |
| ├ 八个内核 crate | ~26k |
| └ 其中助手（`assistant/`） | ~7k |
| 前端测试 `tests/` | ~17k |
