 ...
 * xterm 走动态 import。
 *
 * 模块注册表是**在启动路径上**把四个模块都拉起来的（见 `shell/registry.ts`），
 * 所以静态 import 会让每个用户无论用不用 SSH 都先下载解析一遍 xterm。
...
function holderEl(): HTMLDivElement {
  if (holder) return holder;
  holder = document.createElement('div');
  holder.className = 'rd-ssh-holder';
...
export class TerminalHub {
  ...
  /**
   * 建一个终端。**必须在调 `client.open` 之前 await 完**。
   *
  ...
  async create(sessionId: string, cols: number, rows: number): Promise<void> {
    ...
    container.className = 'rd-ssh-term';
    container.dataset['testid'] = `ssh-term-${sessionId}`;
    ...
    holderEl().appendChild(container);
  }
  ...
  detach(sessionId: string): void {
    ...
    holderEl().appendChild(entry.container);
  }
