...
import type { Doc, Id, Message, MessageKind, Note, Participant, Theme } from '../core/model';
...
import { findTreeNode } from '../platform/types';
import { describeError } from '../platform/types';
import { basename, dirname, stripExt } from '../platform/path';
...
export class AppStore {
  ...
  private syncDoc(extra: Partial<AppState> = {}): void {
    this.set({ doc: this.history.doc, ...extra });
  }

  private fail(e: unknown): void {
    this.set({ error: describeError(e), busy: false });
  }
  ...
  async openWorkspace(root: string, opts: { silent?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      this.set({ workspaceRoot: root, tree, busy: false, status: opts.silent ? null : `已打开工作区 ${root}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async newDiagram(dir = '', name = '未命名'): Promise<void> {
    ...
    try {
      ...
      this.set({ treeSelection: path, status: `已新建 ${basename(path)}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async newFolder(dir = '', name = '新建文件夹'): Promise<void> {
    ...
    try {
      ...
      this.set({ treeSelection: path, status: `已新建文件夹 ${name}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async renameEntry(path: string, newName: string): Promise<void> {
    ...
    try {
      ...
      if (this.state.currentPath === path) {
        const doc = cmd.setDocTitle(this.history.doc, stripExt(basename(next)));
        this.history.reset(doc);
        this.set({ currentPath: next, doc, dirty: false });
      }
      this.set({ status: `已重命名为 ${basename(next)}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async deleteEntry(path: string): Promise<void> {
    ...
    try {
      ...
      if (this.state.currentPath === path) {
        ...
        this.set({ doc: this.history.doc, currentPath: null, dirty: false, selection: { type: 'none' } });
      }
      await platform.remove(root, path);
      await this.refreshTree();
      this.set({ status: `已删除 ${basename(path)}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async moveEntry(path: string, newDir: string): Promise<void> {
    ...
    try {
      ...
      this.set({ status: `已移动到 ${newDir || '根目录'}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  async save(): Promise<void> {
    ...
    try {
      await platform.writeText(root, currentPath, serializeDoc(this.history.doc));
      this.set({ dirty: false, status: `已保存 ${basename(currentPath)}` });
    } catch (e) {
      this.fail(e);
    }
  }
  ...
  addParticipant(kind: cmd.AddParticipantOptions['kind'] = 'object'): void {
    const { doc, id } = cmd.addParticipant(this.history.doc, { kind });
    this.commit(doc);
  ...
  commitPendingMessage(): void {
    ...
    const r = cmd.insertMessageAtY(this.history.doc, pending.y, {
      kind: pending.kind,
      from: pending.from,
      to: pending.to,
    });
  ...
  truncateActivation(activationId: Id, y: number): void {
    this.commit(cmd.setActivationEndAtY(this.history.doc, activationId, y));
  }

  /** 在指定消息处断开激活条，并在其后新开一段 */
  splitActivation(activationId: Id, atMessageId: Id): void {
    this.commit(cmd.splitActivation(this.history.doc, activationId, atMessageId));
  }

  updateParticipant(id: Id, patch: Partial<Omit<Participant, 'id'>>): void {
    this.commit(cmd.updateParticipant(this.history.doc, id, patch));
  }

  updateMessage(id: Id, patch: Partial<Omit<Message, 'id'>>): void {
    this.commit(cmd.updateMessage(this.history.doc, id, patch));
  }

  updateNote(id: Id, patch: Partial<Omit<Note, 'id'>>): void {
    this.commit(cmd.updateNote(this.history.doc, id, patch));
  }
  ...
  toggleActivation(participantId: Id, messageId: Id): void {
    this.commit(cmd.toggleActivation(this.history.doc, participantId, messageId));
  }

  distributeParticipants(): void {
    this.commit(cmd.distributeParticipants(this.history.doc));
  }

  distributeMessages(): void {
    this.commit(cmd.distributeMessages(this.history.doc));
  }

  setTheme(theme: Theme): void {
    this.commit(cmd.setDocTheme(this.history.doc, theme));
  }

  patchTheme(patch: Partial<Theme>): void {
    this.commit(cmd.patchDocTheme(this.history.doc, patch));
  }

  setTitle(title: string): void {
    this.commit(cmd.setDocTitle(this.history.doc, title));
  }
