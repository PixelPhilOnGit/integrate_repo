 ...
 * 导出选项。
 *
 * 这里刻意**不接收 Theme 对象**，只要已经编译好的 CSS 字符串和背景色。
 * 原因：这个模块在 shared/ 里，是给所有模块用的，不该认识"顺序图的主题"这个
 * 具体概念 —— 否则就成了共享层反向依赖某个模块。
 * 主题怎么变成 CSS 是各模块自己的事，编译好传进来即可。
 */
export interface ExportOptions {
  /** 内容包围盒 —— 决定 viewBox 和导出尺寸 */
  bounds: Rect;
  /** 已经编译好的样式表内容（顺序图模块传 themeToCss(doc.theme)） */
  css: string;
  /** 画布背景色 */
  backgroundColor: string;
  /** 是否画上背景。透明背景贴到深色文档里更好看，但多数人要白底 */
...
/** 从内容包围盒算出导出尺寸 */
export function exportSize(bounds: Rect, padding = 12): { width: number; height: number } {
  return {
