...
import { serializeDoc } from '../core/schema';
import { FILE_EXT } from '../core/model';
import { defaultTheme } from '../core/theme';

const TREE_KEY = 'rustdraw.workspace.v1';
const PREFS_KEY = 'rustdraw.prefs.v1';
...
function seed(): VNode {
  ...
  return dir('', [
    file(`示例${FILE_EXT}`, serializeDoc(doc)),
    dir('归档', [file(`旧版${FILE_EXT}`, serializeDoc(createNewDoc('旧版', defaultTheme())))]),
  ]);
}
...
function toFileNodes(node: VNode, prefix: string): FileNode[] {
  ...
  for (const child of node.children ?? []) {
    ...
    } else if (isDiagramFile(child.name)) {
  ...
  return {
    ...
    async rename(_root: string, path: string, newName: string): Promise<string> {
      return withTree((root) => {
        ...
        // 图文件必须保留扩展名，用户只改主名
        const finalName =
          node.kind === 'file' && !isDiagramFile(clean) ? clean + FILE_EXT : clean;
