...
| **智能体会话** | 可用。分屏跑多个 Claude Code / Codex，**一个工作目录就是一个窗口**（各有各的分屏布局），侧栏点哪条右边就换哪一套；**谁在等你一眼看出来**（窗格边框 + 侧栏队列 + 模块图标角标）。侧栏能**按状态筛会话**（全部 / 需要你 / 在跑）、也能把常用目录**置顶**。「新建会话」可以一次填几个、建完自动铺成网格，最下面能设**全局启动参数**（如 `--dangerously-skip-permissions`）；父窗口右键可以一次收掉它里面全部会话。状态检测要往 Claude Code / Codex 的配置里装一个钩子，向导里能看到改了什么、随时撤销 |
...
数量（会随开发变动，看实际输出为准）：前端单测 ~1015、e2e ~189、Rust ~330。
