...
| **智能体会话** | 可用。分屏跑多个 Claude Code / Codex，**一个工作目录就是一个窗口**（各有各的分屏布局），侧栏点哪条右边就换哪一套；**谁在等你一眼看出来**（窗格边框 + 侧栏队列 + 模块图标角标）。侧栏能**按状态筛会话**（全部 / 需要你 / 在跑）、也能把常用目录**置顶**；工作目录可以是本机的文件夹，也可以是**另一台机器上的目录**（SSH 过去在那边跑，首次连要核对主机密钥指纹）。「新建会话」可以一次填几个、建完自动铺成网格，最下面能设**全局启动参数**（如 `--dangerously-skip-permissions`）；父窗口右键可以一次收掉它里面全部会话。状态检测要往 Claude Code / Codex 的配置里装一个钩子，向导里能看到改了什么、随时撤销 |
| **助手**（自研 LLM agent） | 可用。说一句话，它在工作区里读文件 / 改文件 / 跑命令，**要动你的东西之前一定先弹审批**（授权粒度是「哪个工具 + 哪个目标」—— 记住 `git` 不等于记住 `curl`，跑 shell 解释器则一律不给「记住」）。模型配置**可以有好几份**（公司的 Anthropic / 自己的 DeepSeek 各一份，各用各的 API key），配好之后有个「测试连接」按钮一次往返确认通不通。⚠️ 和上面那个「智能体会话」**不是一回事**：那个跑的是别人的 CLI、上下文归那些 CLI 管；这个跑的是我们自己的循环，所以上下文策略才可配 |
| **任务** | 可用。一件事从「待办」到「完成」的账本：标题 / 描述 / 状态 / 备注，外加**进度记录**（一条条带时间戳的「什么时候干了什么」）和**归档**（做完的从列表里收起来，一条不删，回顾时看得到全程）。卡片式列表 + 搜索 + 状态筛选，存在本机一个 SQLite 文件里。检查器里能把它**派给某个 agent 会话**（任务内容直接送进那个终端，进度里自动记两笔） |
...
│   ├── agents/              智能体会话内核：本机进程 / 远端会话、状态事件目录、集成配置读写
...
数量（会随开发变动，看实际输出为准）：前端单测 ~1020、e2e ~191、Rust ~330。
...
三个连接类模块（Redis / SQL / SSH）的连接密码**走操作系统的凭据存储**：
Windows 的凭据管理器 / macOS 的钥匙串 / Linux 的 Secret Service。
SSH 多一个**私钥口令**，一样进钥匙串（两个字段打包成一条）。

⚠️ **分三条路走，而且界面上看得见**：

| 环境 | 密码存哪 |
|---|---|
| 桌面端（有钥匙串） | 系统凭据库 —— 本地文件里只剩主机、端口、用户名 |
| 桌面端（**没有**钥匙串：服务器、headless） | 退回存在本地文件里，**侧栏上会挂一个 ⚠** |
| 浏览器版 | 本地（localStorage）—— 那里本来就没有钥匙串，不挂那个 ⚠ |

读写收敛在一处：**`src/shared/connections/profiles.ts`**，所以三个模块
**一行都没改**（除了侧栏那个提示）—— 这正是当初把这层抽出来的主要理由。
钥匙串那头在 `src-tauri/store/src/secrets.rs`（`keyring` crate）加一个很薄的命令层。

> ⚠️ **老数据会自动搬**：第一次读的时候，本地文件里还剩明文密码的那些会被搬进钥匙串，
> **搬成功了才**从文件里删。搬不进去就一个字都不动（下次再试）——
> 你的密码不会因为一次升级没了。
