...
impl PtySession {
    ...
    pub fn close(&self) {
        ...
        // ⚠️ **拿不到锁就直接往下走，绝不在这儿等。**
        //
        // 写 PTY 是阻塞的，而且 Windows 上会**无限期**阻塞（ConPTY 的输入是根
        // 4KB 的阻塞管道，对面不读输入时写就卡住），那个线程会一直握着这把锁。
        // 关窗格和「应用退出时的 close_all」都走这条路 —— 在这儿等，表现就是
        // **点了关闭没反应，只能杀进程**（真机上出过）。
        // 拿不到说明有人正卡在写里：主端一丢，管道自然断，那个线程也会结束。
        ...
        // 那比卡住退出流程好。
        // 这里也一样：master 的锁可能被 resize 握着（而 `ResizePseudoConsole`
        // 在旧版 Windows 上同样会等）。拿不到就不丢 —— 那意味着要等，而这条路
        // 的调用方可能是**应用退出**。留一个句柄比卡住退出强。
...
fn maybe_send_exit(shared: &Shared, tx: &tokio::sync::mpsc::Sender<PtyEvent>) {
    ...
    // ⚠️ **有上限地等**，不能 `blocking_send` 到底。
    //
    // 通道满了意味着没人取（转发任务被卡住、或者运行时整个堵住了）。这时候
    // `blocking_send` 会**永远**等下去 —— 这个线程（`pty-wait`）就再也退不出来，
    // 而「把会话从表里摘掉」（`forget`）是等这条 Exit 才发生的：结果是**死掉的
    // 会话永远留在表里**，连同 PTY 句柄、作业对象、线程一起。
    //
    // 所以我们等 2 秒，等不到就放弃（丢一条退出事件 = 界面上那个会话停不到
    // 「已退出」，但进程树已经由作业对象管着，不会变成孤儿）。
    // 先 `try_send` 一次是常态路径：大多数时候通道是空的，一次就成。
    let deadline = std::time::Instant::now() + Duration::from_secs(2);
    loop {
        match tx.try_send(PtyEvent::Exit { code }) {
            Ok(()) => return,
